...
export default function DashboardPage() {
  ...
  const bg = { minHeight: "100vh", background: "linear-gradient(135deg,#1e1b4b,#0f172a,#064e3b)", padding: "1.5rem", fontFamily: "sans-serif" };
  ...
  if (role === "admin" && data) return (
    <div style={bg}>
      <div style={{ marginBottom: "1.5rem" }}>
        <h1 style={{ color: "#e0e7ff", fontSize: "1.5rem", fontWeight: 700, margin: 0 }}>Admin Dashboard</h1>
        <p style={{ color: "rgba(255,255,255,0.4)", fontSize: "0.85rem", margin: "4px 0 0" }}>Welcome back! Here's what's happening today.</p>
      </div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(130px,1fr))", gap: "0.875rem", marginBottom: "1.25rem" }}>
        {[
          ...
          <div key={s.label} style={{ ...glass, display: "flex", flexDirection: "column", gap: 4 }}>
            <span style={{ fontSize: "0.68rem", color: "rgba(255,255,255,0.4)", textTransform: "uppercase", letterSpacing: "0.05em" }}>{s.label}</span>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "1rem", marginBottom: "1.25rem" }}>
        <div style={glass}>
          <p style={{ color: "rgba(255,255,255,0.5)", fontSize: "0.75rem", margin: "0 0 0.875rem", textTransform: "uppercase" }}>Asset Status</p>
          <ResponsiveContainer width="100%" height={180}>
            <PieChart>
              ...
              ]} cx="50%" cy="50%" innerRadius={45} outerRadius={72} dataKey="value">
        ...
        <div style={glass}>
          <p style={{ color: "rgba(255,255,255,0.5)", fontSize: "0.75rem", margin: "0 0 0.875rem", textTransform: "uppercase" }}>By Category</p>
          <ResponsiveContainer width="100%" height={180}>
      ...
      {/* Recent Issues */}
      <div style={glass}>
        <p style={{ color: "rgba(255,255,255,0.5)", fontSize: "0.75rem", margin: "0 0 0.875rem", textTransform: "uppercase" }}>Recent Issues</p>
        ...
        {data.recent_issues?.map((issue: any) => (
          <div key={issue.id} style={{ display: "flex", alignItems: "center", gap: "0.875rem", padding: "0.75rem 0", borderBottom: "1px solid rgba(255,255,255,0.06)" }}>
            ...
            <div style={{ flex: 1, minWidth: 0 }}>
              <p style={{ color: "#e2e8f0", fontWeight: 600, margin: 0, fontSize: "0.875rem", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{issue.asset_name}</p>
              <p style={{ color: "rgba(255,255,255,0.4)", fontSize: "0.75rem", margin: "2px 0 0", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{issue.issue_description}</p>
              <p style={{ color: "rgba(255,255,255,0.25)", fontSize: "0.7rem", margin: "2px 0 0" }}>by {issue.employee_name}</p>
            </div>
            <span style={{
  ...
  return (
    <div style={bg}>
      ...
      <div style={{ ...glass, display: "flex", alignItems: "center", gap: "1rem", marginBottom: "1.25rem" }}>
        ...
        <div>
          <p style={{ color: "#e0e7ff", fontWeight: 700, fontSize: "1.1rem", margin: 0 }}>Welcome, {data?.full_name || "Employee"}!</p>
          <p style={{ color: "rgba(255,255,255,0.4)", fontSize: "0.8rem", margin: "2px 0 0" }}>{data?.email}</p>
        </div>
      </div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(3,1fr)", gap: "0.75rem", marginBottom: "1.25rem" }}>
        {[
          ...
          <div key={s.label} style={{ ...glass, textAlign: "center" }}>
            ...
            <div style={{ fontSize: "0.68rem", color: "rgba(255,255,255,0.4)", marginTop: 4, textTransform: "uppercase" }}>{s.label}</div>
          </div>
        ))}
      </div>
      ...
      <div style={{ ...glass, marginBottom: "1.25rem" }}>
        <p style={{ color: "rgba(255,255,255,0.5)", fontSize: "0.75rem", margin: "0 0 0.875rem", textTransform: "uppercase" }}>My Assigned Assets</p>
        {(!data?.my_assets || data.my_assets.length === 0) ? (
          <p style={{ color: "rgba(255,255,255,0.3)", fontSize: "0.85rem" }}>No assets assigned to you yet.</p>
        ) : (
          data.my_assets.map((a: any, i: number) => (
            <div key={i} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "0.75rem 0", borderBottom: "1px solid rgba(255,255,255,0.06)" }}>
              <div>
                ...
                <p style={{ color: "rgba(255,255,255,0.4)", fontSize: "0.72rem", margin: "2px 0 0" }}>{a.asset_code} · {a.asset_category}</p>
              </div>
              <div style={{ textAlign: "right" }}>
                <span style={{ fontSize: "0.68rem", padding: "2px 8px", borderRadius: 999, background: "rgba(99,102,241,0.15)", color: "#818cf8", fontWeight: 600 }}>assigned</span>
                <p style={{ color: "rgba(255,255,255,0.25)", fontSize: "0.68rem", margin: "4px 0 0" }}>Since {new Date(a.assigned_date).toLocaleDateString()}</p>
              </div>
            </div>
          ))
        )}
      ...
      {/* Recent Issues */}
      <div style={glass}>
        <p style={{ color: "rgba(255,255,255,0.5)", fontSize: "0.75rem", margin: "0 0 0.875rem", textTransform: "uppercase" }}>My Recent Issues</p>
        {(!data?.recent_issues || data.recent_issues.length === 0) ? (
          <p style={{ color: "rgba(255,255,255,0.3)", fontSize: "0.85rem" }}>No issues reported yet.</p>
        ) : (
          data.recent_issues.map((issue: any) => (
            <div key={issue.id} style={{ display: "flex", alignItems: "center", gap: "0.875rem", padding: "0.75rem 0", borderBottom: "1px solid rgba(255,255,255,0.06)" }}>
              ...
              <div style={{ flex: 1, minWidth: 0 }}>
                ...
                <p style={{ color: "rgba(255,255,255,0.4)", fontSize: "0.72rem", margin: "2px 0 0", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{issue.issue_description}</p>
              </div>
              <span style={{
                fontSize: "0.68rem", padding: "2px 8px", borderRadius: 999, fontWeight: 600, flexShrink: 0,
                background: issue.issue_status === "open" ? "rgba(244,63,94,0.15)" : "rgba(16,185,129,0.15)",
